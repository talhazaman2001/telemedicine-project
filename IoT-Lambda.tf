        ...
        Statement = [
            {
                ...
                Resource = aws_lambda_function.process_glucose_levels_lambda.arn
            }
        ]
    })
}
...
resource "aws_iam_role_policy_attachment" "iot_lambda_policy_attach" {
    ...
    policy_arn = aws_iam.policy.iot_invoke_lambda_policy.arn
}
...
resource "aws_lambda_function" "process_glucose_levels" {
    ...
    environment {
      variables = {
        TABLE_NAME = aws_dynamodb_table.glucose_levels.name
      }
    }
}
